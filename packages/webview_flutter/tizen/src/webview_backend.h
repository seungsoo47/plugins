 ...
 public:
  ...
  class Delegate {
   public:
    ...
    virtual void OnNavigationPolicyDecide(const std::string& url) = 0;
    virtual void OnResponsePolicyDecide(const std::string& url,
                                        int32_t status_code) = 0;
    virtual void OnUrlChanged(const std::string& url) = 0;
    virtual void OnJavaScriptMessage(const std::string& channel,
                                     const std::string& message) = 0;
  ...
  virtual void SetHasNavigationDelegate(bool has_navigation_delegate) = 0;
  ...
  virtual void ScrollBy(int32_t x, int32_t y) = 0;
